...
function perform_backups()
{
	...
	TODAYS_DATE=`\%Y-\%m-\%d_\%H_\%M_\%S`
	SUFFIX=$1
	FINAL_BACKUP_DIR=$BACKUP_DIR"`date +\%Y-\%m-\%d_\%H_\%M_\%S`$SUFFIX/"
	...
	then
		    ...
		    if ! PGPASSWORD=$PASSWORD pg_dumpall -g -h "$HOSTNAME" -U "$USERNAME" | gzip > $FINAL_BACKUP_DIR"globals".sql.gz.in_progress; then
		            ...
		            echo "$TODAYS_DATE Postgres backup script execution error" | mail -s "Postgres backup ERROR" $MAIL_RECIPIENTS
	...
	if [ "$IS_BACKUP_SUCCESS" = true ] ; then
		echo "$TODAYS_DATE Postgres backup script executed successfully" | mail -s "Postgres backup success" $MAIL_RECIPIENTS
	fi
