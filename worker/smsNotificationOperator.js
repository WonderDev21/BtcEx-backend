...
class smsNotificationOperator extends Operator {
  constructor(frequencyExpression) {
    super(constants.DAILY_SMS, frequencyExpression);
  }
  run() {
    console.log('queing operation ', this.taskType);
    logger.info(`Running operator for ${this.taskType}`);
    const users = [{email: 'parag99799@gmail.com', password: '1', mobile:'+919599622943'}];
    // User.find({})
    // .then((users) => {
    _.forEach(users, user => {
      const task = queue.create(constants.DAILY_SMS, user);
...
module.exports = smsNotificationOperator;
